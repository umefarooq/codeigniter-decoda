...
define('DECODA_GESHI', DECODA .'decoda/geshi'. DIRECTORY_SEPARATOR);
define('DECODA_EMOTICONS', DECODA .'decoda/emoticons'. DIRECTORY_SEPARATOR);
define('DECODA_CONFIG', DECODA .'decoda/config'. DIRECTORY_SEPARATOR);

// Includes
include_once DECODA .'decoda/decodaconfig.php';
...
class Decoda {
    ...
    public function __construct($string = '', $allowed = array()) {
        ...
        $this->codaconfig = new DecodaConfig();
        $this->ci =& get_instance();

        if($this->ci->config->load('decoda',TRUE,TRUE)){

                    log_message('debug', 'Decoda: config loaded from config file.');

                    $config = $this->ci->config->item('decoda');
                    $this->config($config);
        }
    ...
    function config($config = array()){
			...
			log_message('error', 'Assets Config File is Missing or Empty');
		}
    }
...
//        if($string)
        ...
        if (!$this->__config['parse']) {
			...
			$markup = $this->codaconfig->markup();
			$markupResult = $this->codaconfig->markup(true);
    ...
    private function __censor() {
		$censored = $this->codaconfig->censored();
    ...
    private function __email($matches, $isCallback = false) {
        ...
        if ($this->__config['shorthand']) {
            return $padding .'[<a href="mailto:'. $encrypted .'">'. $this->codaconfig->message('mail') .'</a>]';
        } else {
    ...
    private function __emoticons() {
		$emoticons = $this->codaconfig->emoticons();
    ...
    private function __spoiler($matches) {
		...
		$showText = $this->codaconfig->message('spoiler') .' ('. $this->codaconfig->message('show') .')';
		$hideText = $this->codaconfig->message('spoiler') .' ('. $this->codaconfig->message('hide') .')';
    ...
    private function __url($matches, $isCallback = false) {
        ...
        if ($this->__config['shorthand']) {
            return $padding .'[<a href="'. $url .'">'. $this->codaconfig->message('link') .'</a>]';
        } else {
